...
class SingleModel(torch.nn.Module):
    def __init__(self,n_channels=2,n_classes=19,da=False):
        super().__init__()

        self.n_channels = n_channels

        self.backbone = models.resnet50(pretrained=False)
        self.backbone.conv1 = torch.nn.Conv2d(n_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        self.backbone.fc = torch.nn.Linear(2048,n_classes)
    ...
    def forward_backbone(self, x, da_l3, da_l4):
    ...
    def forward(self,s1):
        if self.n_channels==2:
            return self.forward_backbone(s1, self.da1_l3, self.da1_l4)
        elif self.n_channels==13:
            return self.forward_backbone(s1, self.da2_l3, self.da2_l4)
...
def main():
    ...
    if args.backbone == 'resnet50':
        if args.mode==['s1']:
            net = SingleModel(n_channels=2,n_classes=19,da=args.rda)
            net.backbone.fc.weight.data.normal_(mean=0.0,std=0.01)
            net.backbone.fc.bias.data.zero_()   
        if args.mode==['s2']:
            net = SingleModel(n_channels=13,n_classes=19,da=args.rda)
            net.backbone.fc.weight.data.normal_(mean=0.0,std=0.01)
            net.backbone.fc.bias.data.zero_()
        if args.mode==['s1','s2']:
            net = LateFusionModel(args.rda)            
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            if args.mode==['s1']:
                ...
                assert set(msg1.missing_keys) == {"backbone.fc.weight", "backbone.fc.bias"}            
            if args.mode==['s2']:
                checkpoint2 = torch.load(args.pretrained, map_location="cpu")
                state_dict2 = checkpoint2['model'] if 'model' in checkpoint1 else checkpoint1
                ...
                assert set(msg2.missing_keys) == {"backbone.fc.weight", "backbone.fc.bias"}
            if args.mode==['s1','s2']:
                checkpoint = torch.load(args.pretrained, map_location="cpu")
                state_dict = checkpoint['model'] if 'model' in checkpoint1 else checkpoint1
