...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        #### ATTENZIONE!! questa è la vera modifica:
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            arch = getattr(self, "arch", None)  # <--- usa l'arch memorizzata
            ...
            raise TypeError("pretrained must be a str, True, or None")


def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet(block, layers, **kwargs)
    model.arch = arch # not best practice definire dinamicamente attributo classe così però ok (chill)
    #  if pretrained:
