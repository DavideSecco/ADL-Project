...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        # Aggiunto weights_only=False perchè da pytorch 2.6 il comportamento di default è cambiato
...
#
    ...
    # Start training
    # QUI PRIMA C'ERA TUTTO IL TRAINING
    ...
    if rank in [-1, 0]:
        ...
        for m in ((best,) if best.exists() else (last,)):
            ...
            results, _, MRresult, _ = test.test(opt.data,
                                                batch_size=1, # da verificare se 1 o batch_size, da cambiare anche sopra!
                                                imgsz=imgsz_test,
                                                conf_thres=0.001,
                                                iou_thres=0.5,
